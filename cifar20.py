...
STEPS_EVALUATION = math.ceil(NUM_OF_TEST_SAMPLES//BATCH_SIZE)
...
with strategy.scope():
    model = build_model()
model.fit(dataset_train, epochs=2, steps_per_epoch=STEPS_PER_EPOCH)

# Show model summary, and evaluate it
model.summary()
eval_loss, eval_acc = model.evaluate(dataset_test, steps=STEPS_EVALUATION)
